...
<div class="app-card app-card-accordion shadow-sm mb-4">
    <div class="app-card-body p-4">
        ...
        <form action="<?= base_url('bp/pelanggaran/save') ?>" method="POST">
            <div class="row">
                <div class="col-md-3">
                    <strong>Pilih siswa dan Pelanggaran:</strong>
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-3">
                    <select name="poin" class="form-select selectPoin" style="width:100%" required>
                        <option value="">Pilih Pelanggaran</option>
                        <?php foreach ($poin as $p) : ?>
                            <option value="<?= $p->id ?>"><?= $p->pelanggaran ?> &bull; <?= $p->poin ?> poin</option>
                        <?php endforeach ?>
                    </select>
                </div>
                <div class="col-md-1">
...
<script>
    ...
    $(document).ready(function() {
        $('#table').DataTable({
            processing: true,
            serverSide: true,
            ajax: '<?= base_url('bp/pelanggaran/data') ?>',
            order: [],
            columns: [{
                    data: 'no',
                    orderable: false
