...
<div class="app-card app-card-accordion shadow-sm mb-4">
    <div class="app-card-body p-4">
        <form action="<?= base_url('admin/laporan_rekap_mapel') ?>" method="GET">
        <input type="text" name="id_mk" value="<?=$_GET['id_mk'];?>" class="form-control" required>
            ...
            <div class="row">
                ...
                <div class="col-md-2">
                    ...
                    <input type="text" name="kelas" class="form-control my-2" value="<?= $_GET['kelas'] ?>" required readonly>
                </div>
                <div class="col-md-2">
                                ...
                                <tbody>
                                    <?php if (!empty($siswa)) : ?>
                                        ...
                                        <?php foreach ($siswa as $siswa) : ?>
                                            <?php
                                            $th = $absensi->where(['MONTH(tanggal)' => $bln[1], 'YEAR(tanggal)' => $bln[0], 'siswa' => $siswa->id, 'absensi' => 'h','id_mengajar' => $_GET['id_mk'], 'tahun_pelajaran' => $_GET['tahun_pelajaran'], 'semester' => $_GET['semester'], 'kelas' => $_GET['kelas']])->countAllResults();
                                            $ts = $absensi->where(['MONTH(tanggal)' => $bln[1], 'YEAR(tanggal)' => $bln[0], 'siswa' => $siswa->id, 'absensi' => 's', 'id_mengajar' => $_GET['id_mk'],'tahun_pelajaran' => $_GET['tahun_pelajaran'], 'semester' => $_GET['semester'], 'kelas' => $_GET['kelas']])->countAllResults();
                                            $ti = $absensi->where(['MONTH(tanggal)' => $bln[1], 'YEAR(tanggal)' => $bln[0], 'siswa' => $siswa->id, 'absensi' => 'i', 'id_mengajar' => $_GET['id_mk'],'tahun_pelajaran' => $_GET['tahun_pelajaran'], 'semester' => $_GET['semester'], 'kelas' => $_GET['kelas']])->countAllResults();
                                            $ta = $absensi->where(['MONTH(tanggal)' => $bln[1], 'YEAR(tanggal)' => $bln[0], 'siswa' => $siswa->id, 'absensi' => 'a', 'id_mengajar' => $_GET['id_mk'],'tahun_pelajaran' => $_GET['tahun_pelajaran'], 'semester' => $_GET['semester'], 'kelas' => $_GET['kelas']])->countAllResults();
                                    ...
                                    <?php for ($a = 1; $a <= 31; $a++) : ?>
                                        <td style="padding:0">
                                            <table class="table table-sm" style="margin: 0;">
                                                <?php
                                                $tgl = $_GET['bulan'] . '-' . $a;
                                                $q = $absensi->where(['tanggal' => $tgl, 'kelas' => $_GET['kelas'],'id_mengajar' => $_GET['id_mk'], 'tahun_pelajaran' => $_GET['tahun_pelajaran'], 'semester' => $_GET['semester']])->orderBy('siswa', 'ASC')->findAll();
                                                if (!empty($q)) {
                                                    $no = 1;
                                                    $abs = '';
                                                    $h = 0;
