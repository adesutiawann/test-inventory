    ...
    public function importxx()
    {
        ...
        if ($extension == 'xlsx' || $extension == 'xls') {
            ...
            foreach ($contak as $key => $value) {
                ...
                // Check if the data already exists in the database
                $existingData = $this->laptop->where('serial', $data['serial'])->first();

                if (!$existingData) {
                    // Data doesn't exist, proceed with insertion
                    $insertSuccess = $this->laptop->insert($data);
                } else {
                    // Data already exists, set importSuccess to false
                    $importSuccess = false;
                    session()->setFlashdata('warning', '<strong>Peringatan!</strong> Data dengan nomor serial <b>' . $data['serial'] . '</b> sudah terdaftar.');
                    break; // Exit the loop if any data already exists
                }
            }
